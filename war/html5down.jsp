...
<html>
	<head>
		...
		<script type="text/javascript" src="../../html5down.js"></script>
		<script type="text/javascript">
			...
			var serverlist = '<%= (String)request.getAttribute("serverlist") %>'.split('|');
			var blobkeylist = '<%= (String)request.getAttribute("blobkeylist") %>'.split('|');
			...
			function updateState(state){
				if(state == 'init'){
					...
					e_div_download.style.display = 'none';
	...
	<body style="margin:0px;" onload="init();" onunload="html5Close();">
			...
			<div style="width:62%; height:150px; margin:0px auto; background-color:#FDFDFD; border:solid 1px #D9D9D9; text-align:center;">
				<table style="width:62%; height:100px; margin:0px auto; text-align:left;"><tr>
					...
					<td style="width:100px;">
						<div id="div_download" class="button_r" style="width:100px; height:50px; line-height:50px;" onmouseover="this.className='button_r_hi';" onmouseout="this.className='button_r';" onclick="
							if(state == 'init'){
								state = 'run';
								e_div_download.firstChild.nodeValue = '取消';
								
								window.webkitStorageInfo.requestQuota(TEMPORARY,8192*1024*1024,function(size){
									html5Init();
								},function(err){
									console.log(e);
								});
								...
								state = 'cancel';
								e_div_download.firstChild.nodeValue = '下載檔案';
							}
						">下載檔案</div>
					</td>
				</tr></table>
		...
		<a id='a_html5test' style="display:none"></a>
	</body>
</html>
